...
class GetFeat {
  public async getFeat(): Promise<IFeat[]> {
    ...
    const results = await Database.query(query, [],).catch((e,) => {
      throw e;
    },);
  ...
  public async getFeatByID(id: any,): Promise<IFeat> {
    ...
    const parameters = [ id, ];
    const results = await Database.query(query, parameters,).catch((e,) => {
      throw e;
    },);
  ...
  public async getReleatedFeats(id: any,): Promise<IFeatRelated> {
    ...
    const relatedFeats: IFeatRelated = {required_for: [], requires: [], same_feat: [], };

    const sameFeatQuery =
      getFeatLiteQuery +
      `WHERE f.id != $1 and fn.id IN (SELECT fn.id FROM feats.feat_names LEFT JOIN feats.feat_index as fi ON fn.id = fi.feat_name_id WHERE fi.feat_id = $1)`;
    const parameters = [ id, ];
    const results1 = await Database.query(sameFeatQuery, parameters,).catch((e,) => {
      throw e;
    },);

    const results2 = await Database.query(
      getRequiresFeatsQuery,
      parameters,
    ).catch((e,) => {
      throw e;
    },);
